...
function Dungeon:onSuccess(instance)
  ...
  if self:getTitle() == "Underworld" then
    for _, runner in ipairs(runners) do
      if not runner:hasCompletedChallenge(ChallengesIndex.UNDERWORLD_THE_BEGINNING) then
        runner:completeChallenge(ChallengesIndex.UNDERWORLD_THE_BEGINNING)
      end
    end
  end
