  ...
  if target then
    ...
    for slot = CONST_SLOT_HEAD, CONST_SLOT_AMMO do
      ...
      if item then
        ...
        item_data.slot = slot
        player:sendExtendedOpcode(ExtendedOPCodes.CODE_INSPECT, json.encode({action = "item", data = item_data}))
      else
        player:sendExtendedOpcode(ExtendedOPCodes.CODE_INSPECT, json.encode({action = "item", data = {slot = slot, empty = true}}))
      end
    end
  end
end
