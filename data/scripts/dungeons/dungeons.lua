  ...
  if killer:isPlayer() then
    ...
    if dungeon then
      ...
      if instance then
        ...
        if left == 0 then
          if dungeon:getTitle() == "Underworld" then
            for _, runner in ipairs(runners) do
              if not runner:hasCompletedChallenge(ChallengesIndex.UNDERWORLD_MONSTER_HUNTER) then
                runner:completeChallenge(ChallengesIndex.UNDERWORLD_MONSTER_HUNTER)
              end
            end
          end
        end
