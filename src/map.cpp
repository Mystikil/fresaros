...
bool Map::loadDungeon(const std::string& identifier, const Position& pos, bool unload, bool lua, DungeonInstance* instance, uint8_t difficulty)
{
	IOMap loader;
	...
	if (lua && !unload) {
		if (instance->getId() >= chunkSpawns.size())
			chunkSpawns.resize(instance->getId() + 1);

		if (!IOMap::loadChunkSpawns(this, identifier, pos, instance->getId())) {
			std::cout << "[Warning - Map::loadDungeon] Failed to load chunk spawn data." << std::endl;
		}

		chunkSpawns[instance->getId()].startup(instance, difficulty);
	}
	else if (unload) {
		chunkSpawns[instance->getId()].clear();
	}

	if (!lua) {
		if (!IOMap::loadSpawns(this)) {
			std::cout << "[Warning - Map::loadDungeon] Failed to load spawn data." << std::endl;
		}
	}
	return true;
}
