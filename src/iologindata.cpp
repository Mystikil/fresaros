...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	if ((result = db.storeQuery(fmt::format("SELECT `huntingtasks` FROM `players` WHERE `id` = {:d} LIMIT 1", player->getGUID())))) {
		attr = result->getStream("huntingtasks", attrSize);
		propStream.init(attr, attrSize);
		for (uint8_t i = 0; i < 3; ++i) {
			uint8_t taskStatus;
			if (propStream.read<uint8_t>(taskStatus)) {
				player->huntingTasks[i] = HuntingTask();
				player->huntingTasks[i].status = static_cast<HuntingTaskStatus>(taskStatus);
				switch (taskStatus) {
				case HUNTINGTASK_INACTIVE: {
					std::string creatureName;
					for (uint8_t k = 0; k < 9; ++k) {
						propStream.readString(creatureName);
						player->huntingTasks[i].creatures.emplace_back(creatureName);
					}
					break;
				}
				case HUNTINGTASK_ACTIVE: {
					std::string creatureName;
					uint8_t stars;
					uint16_t kills;
					uint16_t amount;
					propStream.readString(creatureName);
					propStream.read<uint8_t>(stars);
					propStream.read<uint16_t>(kills);
					propStream.read<uint16_t>(amount);
					player->huntingTasks[i].creature = std::move(creatureName);
					player->huntingTasks[i].stars = stars;
					player->huntingTasks[i].kills = kills;
					player->huntingTasks[i].amount = amount;
					break;
				}
				default:
					break;
...
bool IOLoginData::savePlayer(Player* player)
{
  ...
  propWriteStream.clear();
	for (uint8_t i = 0; i < 3; ++i) {
		HuntingTaskStatus taskStatus = player->huntingTasks[i].status;
		propWriteStream.write<uint8_t>(taskStatus);
		switch (taskStatus) {
			case HUNTINGTASK_INACTIVE: {
				for (uint8_t k = 0; k < 9; ++k) {
					propWriteStream.writeString(player->huntingTasks[i].creatures[k]);
				}
				break;
			}
			case HUNTINGTASK_ACTIVE: {
				propWriteStream.writeString(player->huntingTasks[i].creature);
				propWriteStream.write<uint8_t>(player->huntingTasks[i].stars);
				propWriteStream.write<uint16_t>(player->huntingTasks[i].kills);
				propWriteStream.write<uint16_t>(player->huntingTasks[i].amount);
				break;
			}
			default:
				break;
  ...
  query.str("");
	...
	const char* attributes = propWriteStream.getStream(attributesSize);
	if (attributesSize > 0) {
		query << "UPDATE `players` SET `huntingtasks` = " << db.escapeBlob(attributes, attributesSize);
	}
	else {
		query << "UPDATE `players` SET `huntingtasks` = NULL";
	}
	query << " WHERE `id` = " << player->getGUID();
