...
void LuaScriptInterface::registerFunctions()
{
  ...
  registerEnum(STAT_MAGICPOINTS)
	...
	registerEnum(HUNTINGTASK_EMPTY)
	registerEnum(HUNTINGTASK_INACTIVE)
	registerEnum(HUNTINGTASK_ACTIVE)
  ...
  registerMethod("Item", "getRealUID", LuaScriptInterface::luaItemGetRealUID);
	...
	registerMethod("Player", "getHuntingTask", LuaScriptInterface::luaPlayerGetHuntingTask);
	registerMethod("Player", "getHuntingTasks", LuaScriptInterface::luaPlayerGetHuntingTasks);
	registerMethod("Player", "setHuntingTaskStatus", LuaScriptInterface::luaPlayerSetHuntingTaskStatus);
	registerMethod("Player", "resetHuntingTaskCreatures", LuaScriptInterface::luaPlayerResetHuntingTaskCreatures);
	registerMethod("Player", "addHuntingTaskCreature", LuaScriptInterface::luaPlayerAddHuntingTaskCreature);
	registerMethod("Player", "setHuntingTaskCreature", LuaScriptInterface::luaPlayerSetHuntingTaskCreature);
	registerMethod("Player", "setHuntingTaskStars", LuaScriptInterface::luaPlayerSetHuntingTaskStars);
	registerMethod("Player", "setHuntingTaskAmount", LuaScriptInterface::luaPlayerSetHuntingTaskAmount);
	registerMethod("Player", "setHuntingTaskKills", LuaScriptInterface::luaPlayerSetHuntingTaskKills);
...
int LuaScriptInterface::luaPlayerGetHuntingTask(lua_State* L)
{
	// player:getHuntingTask(slotId)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		uint8_t slotId = getNumber<uint8_t>(L, 2) - 1;
		lua_createtable(L, 0, 6);

		setField(L, "status", player->huntingTasks[slotId].status);

		int index = 0;
		lua_createtable(L, player->huntingTasks[slotId].creatures.size(), 0);
		for (uint8_t k = 0; k < player->huntingTasks[slotId].creatures.size(); ++k) {
			pushString(L, player->huntingTasks[slotId].creatures[k]);
			lua_rawseti(L, -2, ++index);
		}
		lua_setfield(L, -2, "creatures");

		setField(L, "creature", player->huntingTasks[slotId].creature);
		setField(L, "stars", player->huntingTasks[slotId].stars);
		setField(L, "kills", player->huntingTasks[slotId].kills);
		setField(L, "amount", player->huntingTasks[slotId].amount);
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerGetHuntingTasks(lua_State* L)
{
	// player:getHuntingTasks()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		int index = 0;
		...
		for (uint8_t i = 0; i < 3; ++i) {
			...
			setField(L, "status", player->huntingTasks[i].status);

			int index2 = 0;
			lua_createtable(L, player->huntingTasks[i].creatures.size(), 0);
			for (uint8_t k = 0; k < player->huntingTasks[i].creatures.size(); ++k) {
				pushString(L, player->huntingTasks[i].creatures[k]);
				lua_rawseti(L, -2, ++index2);
			}
			lua_setfield(L, -2, "creatures");

			setField(L, "creature", player->huntingTasks[i].creature);
			setField(L, "stars", player->huntingTasks[i].stars);
			setField(L, "kills", player->huntingTasks[i].kills);
			setField(L, "amount", player->huntingTasks[i].amount);
...
int LuaScriptInterface::luaPlayerSetHuntingTaskStatus(lua_State* L)
{
	// player:setHuntingTaskStatus(slotId, status)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		uint8_t slotId = getNumber<uint8_t>(L, 2) - 1;
		HuntingTaskStatus status = getNumber<HuntingTaskStatus>(L, 3);
		player->huntingTasks[slotId].status = status;
		pushBoolean(L, true);
	}
	else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerResetHuntingTaskCreatures(lua_State* L)
{
	// player:resetHuntingTaskCreatures(slotId)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		uint8_t slotId = getNumber<uint8_t>(L, 2) - 1;
		player->huntingTasks[slotId].creatures.clear();
		pushBoolean(L, true);
	}
	else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerAddHuntingTaskCreature(lua_State* L)
{
	// player:addHuntingTaskCreature(slotId, creatureName)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		uint8_t slotId = getNumber<uint8_t>(L, 2) - 1;
		std::string creatureName = getString(L, 3);
		if (player->huntingTasks[slotId].creatures.size() >= 9) {
			lua_pushnil(L);
			return 1;
		}

		player->huntingTasks[slotId].creatures.emplace_back(creatureName);
		pushBoolean(L, true);
	}
	else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerSetHuntingTaskCreature(lua_State* L)
{
	// player:setHuntingTaskCreature(slotId, creatureName)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		uint8_t slotId = getNumber<uint8_t>(L, 2) - 1;
		std::string creatureName = getString(L, 3);
		player->huntingTasks[slotId].creature = std::move(creatureName);
		pushBoolean(L, true);
	}
	else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerSetHuntingTaskStars(lua_State* L)
{
	// player:setHuntingTaskStars(slotId, stars)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		uint8_t slotId = getNumber<uint8_t>(L, 2) - 1;
		uint8_t stars = getNumber<uint8_t>(L, 3);
		player->huntingTasks[slotId].stars = std::min<uint8_t>(stars, 5);
		pushBoolean(L, true);
	}
	else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerSetHuntingTaskAmount(lua_State* L)
{
	// player:setHuntingTaskAmount(slotId, amount)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		uint8_t slotId = getNumber<uint8_t>(L, 2) - 1;
		uint16_t amount = getNumber<uint16_t>(L, 3);
		player->huntingTasks[slotId].amount = amount;
		pushBoolean(L, true);
	}
	else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerSetHuntingTaskKills(lua_State* L)
{
	// player:setHuntingTaskKills(slotId, kills)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		uint8_t slotId = getNumber<uint8_t>(L, 2) - 1;
		uint16_t kills = getNumber<uint16_t>(L, 3);
		player->huntingTasks[slotId].kills = kills;
		pushBoolean(L, true);
	}
	else {
		lua_pushnil(L);
