...
void NetworkMessage::addItem(const Item* item)
{
  ...
  if (item->isPickupable() && item->isMoveable()) {
		...
		if (rarity) {
			add<uint16_t>(1); // amount of data
			add<uint16_t>(1); // key
			add<uint64_t>(rarity->getInt()); // value

			// adding two different data would look like this
			/*
			add<uint16_t>(2); // amount of data
			
			add<uint16_t>(1); // key
			add<uint64_t>(123); // value
			
			add<uint16_t>(2); // key
			add<uint64_t>(456); // value
			*/
		}
		else {
			add<uint16_t>(0x00);
