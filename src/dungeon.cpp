...
void DungeonInstance::build(const std::string& mapFile, uint8_t difficulty)
{
    ...
    g_dispatcher.addTask(createTask([path, position, unload, instance, difficulty]() {
        try {
            g_game.loadDungeon(path, position, unload, true, instance, difficulty);
        }
        catch (const std::exception& e) {
            std::cout << "[Error - DungeonInstance::build] Failed to load map: " << e.what() << std::endl;
        }
    }));
}
...
void DungeonInstance::destroy(const std::string& mapFile)
{
    ...
    bool unload = true;
    DungeonInstance* instance = this;
    g_dispatcher.addTask(createTask([path, position, unload, instance]() {
        try {
            g_game.loadDungeon(path, position, unload, true, instance, 1);
        }
        catch (const std::exception& e) {
            std::cout << "[Error - DungeonInstance::destroy] Failed to load map: " << e.what() << std::endl;
        }
    }));
}
